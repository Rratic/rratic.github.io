...
对于 $r = 2$，设 $V^\ast$ 的基 $f_1, \dots, f_{2n}$，令 $L = \sum_{k=1}^n f_{2k-1} \wedge f_{2k}$，则条件即：
...
考虑基 $\alpha_1, \dots, \alpha_n$ 下的对偶 $V^\ast \to V, f \mapsto (f(\alpha_1), \dots, f(\alpha_n))$.
...
假设 $\gcd(x_{n-1}, x_n) = d, x_{n-1} = du, x_n = dv$，设 $\lambda u + \mu v = 1$，则取前 $n-2$ 个向量为 $(x_1, \dots, x_{n-2}, d)$ 对应的向量（最后一位添上 $0$），再取 $(0, \dots, 0, \mu, -\lambda)$ 即可。

---

另一个想法是，结论等价于说构造一个第一行是 $x_1, \dots, x_n$ 的行列式为 $\pm 1$ 的矩阵。
...
在基 $1, x-\beta_1, (x-\beta_1)(x-\beta_2), \dots, (x-\beta_1)\cdots(x-\beta_{n-1})$ 中考虑 $T_1$ 对应的矩阵，是一个上三角阵，行列式为 $f(\beta_1)\cdots f(\beta_n) = \prod (\beta_i - \alpha_j)$. $T_2$ 同理，使用 $n$ 是偶数。
...
[^sylvester-theorem]: 一般的证法是构造分块矩阵。我的想法是，在 $A$ 可逆时可直接用 $A(A^{-1} + B)$ 证明；使用连续性推广到不可逆情形。此外，一个结论说 $I-AB$ 可逆时可以推出 $(I-BA)^{-1} = I + B(I-AB)^{-1}A$, 从而整理式子得 $\det (I-BA) = \det (I-AB)$.

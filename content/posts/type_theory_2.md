...
Bishop 则成功展示了数学的许多重要部分都能在构造主义中发展。Bishop 认为，要构造一个集合，只需要说明如何构造它的元素，并且说明构造出的两个元素如何判断相等。这种定义集合的方式现在被称为 Bishop 集合，在类型论下看这就是一个类型上配备了一个等价关系，称作**广集**（setoid）。对于 $(A, \sim_A), (B, \sim_B)$ 我们可以规定 $A \implies B$ 是在 $A \to B$ 上配备等价关系：

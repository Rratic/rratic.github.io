...
对旋转计数为 $2(n-1)$，对极点计数则得到 $\sum (|Stab(p_i)|-1)|Orb(p_i)|=nk-\sum |Orb(p_i)|$，两式相等。等式即 $2-\frac{2}{n} = k - \sum \frac{1}{|Stab(p_i)|}$

又因为 $|Stab(p_i)|\ge 2$，对这个等式分析可以枚举得到所有 $(k; |Stab(p_1)|, |Stab(p_2)| \cdots |Stab(p_k)|; n)$ 的情形如下。

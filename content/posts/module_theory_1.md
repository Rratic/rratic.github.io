...
2. 交换群均可成为 $\Z$ 模：令 $\varphi(m, a) = m + m + \cdots + m$
...
### 自由模
对幺环 $R$ 和 R-模 $M$，它的子集 $S$ 称为 M-**线性无关**的，如果对 $S$ 的任意有限子集 $\set{x_1, x_2, \dots, x_n}$ 有 $x_1a_1 + \cdots + x_na_n = 0$ 蕴含 $a_1 = \cdots = a_n = 0$.
...
设它的一组基 $\set{x_1, x_2, \dots , x_r}$，由 $R$ 是交换幺环，它有极大理想。取一个极大理想 $\mathfrak{M}$，令 $N = x_1\mathfrak{M}+\cdots +x_r\mathfrak{M}$，则它是 $M$ 的一个子模。
...
如果 R-模 $M$ 是自由的，不考虑零模，设有基 $x_1, \dots , x_n$ 及 $xa=0$，有 $x=x_1a_1+\cdots+x_na_n$ 推出 $0=x_1a_1a+\cdots+x_na_na$，故 $a_i$ 全为零，矛盾。

反之，如果是无扭的，设有一组生成元 $x_1, \dots , x_m$，极大线性无关组 $x_1, \dots , x_r$，设它们生成模 $N$.

若 $r < m$，设 $x_1a_{j1} + \cdots + x_ra_{jr} + x_jb_j = 0, j=r+1, \dots, m$，我们令 $b = b_{r+1}\cdots b_m$，有 $Mb\subseteq N$ 为自由模，又 $\varphi: M\to Mb,\ x\mapsto xb$ 为模同构。

### 分解
我们先证明：

{% <theorem title="有限生成自由模的子模"> %}

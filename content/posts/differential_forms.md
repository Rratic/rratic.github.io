...
考虑有序对 $(f, U)$，其中 $U$ 是开集，$f$ 是 $U\to\R$ 的光滑函数，称 $(f, U)$ 与 $(g, V)$ 等价，如果它们在 $p$ 的某个邻域恒等。称这个等价类为 $f$ 在 $p$ 处的**函数芽**，记作 $C_p^\infty (M)$，它构成 $\R$-线性空间并构成结合代数（考虑 $[(f, U)]_ p[(g, V)]_ p = [(fg, U\cap V)]_ p$）。然后就可以以类似方式定义导子。

...
$p$ 处 $C^\infty (M)$ 的全体导子构成 $M$ 在 $p$ 处的**切空间**，易见是线性空间，记作 $T_pM$.
...
考虑有序对 $(f, U)$，其中 $U$ 是开集，$f$ 是 $U \to \R$ 的光滑函数，称 $(f, U)$ 与 $(g, V)$ 等价，如果它们在 $p$ 的某个邻域恒同，称这个等价类为 $f$ 在 $p$ 处的**函数芽**。全体等价类构成一个函数芽环，记作 $C_p^\infty (M)$ 或 $\mathcal{E}(p)$，它构成 $\R$-线性空间且与运算 $[(f, U)]_ p[(g, V)]_ p = [(fg, U\cap V)]_ p$ 构成结合代数。

现在，我们可以给出切空间另一定义为曲线的等价类。考虑 $\gamma(0) = p$ 的曲线，让 $\gamma_1 \sim \gamma_2$，如果对任一函数芽 $[f]$ 有：

...
“交叉帽”是在洞口粘一条 Möbius 带，等效地说是把洞口的对径点粘合。把球面上安 $m$ 个交叉帽得到的称为 $mP^2$ 亏格为 $m$ 的不可定向闭曲面。有 $1P^2$ 就是 $P^2$，$2P^2$ 就是 Klein 瓶。

{% <theorem title="闭曲面分类定理"> %}
...
$$a_1a_1a_2a_2 \cdots a_ma_m \tag{P}$$
...
{% <note title="Wirtinger presentation"> %}
...
{% <theorem title="Nielsen-Schreier 定理"> %}
自由群的子群是自由群。
{% </theorem> %}
...
考虑模群 $\mathrm{SL}(2, \Z)$ 的无挠子群 $\Gamma(2) / \set{\pm I}$，其中：

...
## 正态分布
### 来源
许多分布会趋近于 $\varphi(x) = \frac{1}{\sqrt{2\pi}}\exp (-\frac{x^2}{2})$，我们称为**中心/高斯/正态分布**。它从 $-\infty$ 到 $+\infty$ 的积分结果为 $1$ 可由下式说明：
...
一般是用累积高斯分布 $\Phi(x)=\int_{-\infty}^x \varphi(t) \mathrm{d}t$ 进行计算。

...
{% <theorem title="Newton-Leibniz 定理"> %}
...
{% <question title="习题课 W2 13 Minkowski 不等式"> %}
...
{% <question title="习题课 W3 4 (1)"> %}
$$\lim_{n \to +\infty} \int_{n^2}^{n^2+n} \frac{e^{-1/x}}{\sqrt{x}} = 1$$
{% </question> %}

重点在于想到可以用中值定理。

{% <question title="讲义例 7.5.1"> %}
设 $f \in C[a, b]$, 证明：
...
{% <theorem title="Riemann-Lebesgue 引理"> %}
...
{% <definition title="无穷乘积"> %}
用极限定义。注意积为 $0$ 时定义为发散。
{% </definition> %}

在 $a_n > -1$ 时 $\prod (1+a_n)$ 收敛当且仅当 $\sum \ln (1+a_n)$ 收敛。

{% <question title="2022 P6"> %}

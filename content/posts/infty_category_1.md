...
## 理论基础
### 单纯集的范畴
{% <definition title="单形范畴（simplex category）"> %}
...
{% <definition title="单纯集（simplicial set）"> %}
单纯集是指 $\Delta$ 上的一个预层。
{% </definition> %}
...
{% <definition title="充实范畴（enriched category）"> %}
...
{% <definition title="单纯范畴（simplicial category）"> %}
...
{% <definition title="Serre fibration"> %}
...
{% <definition title="拟范畴（quasi category）"> %}

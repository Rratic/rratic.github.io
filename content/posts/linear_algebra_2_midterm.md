...
\begin{aligned}
	...
	(\alpha_1, \dots, \alpha_k) & \mapsto \alpha_1 + \cdots + \alpha_k
\end{aligned}
$$
...
取 $T$ 在基 $\mathcal{B} = \set{\alpha_1, \dots, \alpha_n}$ 下的矩阵为 $A$.

先证断言：对 $g \in F[x]$ 如果存在 $B \in F[x]^{n\times n}$ 使得 $(xI_n-A)B = gI_n$, 则 $g(T) = 0$. 这是因为：

$$(0, \dots, 0) = ((\alpha_1, \dots, \alpha_n)(xI_n - A)) B = (\alpha_1, \dots, \alpha_n)(gI_n)$$
...
令 $T_i = T_{W_i}$, 读者易证 $p_T = \operatorname{lcm}(p_{T_1}, \dots, p_{T_k})$ 及：
...
易见 $T$ 循环当且仅当 $p_T = f_T$, 且在基 $\set{\alpha, T\alpha, \dots, T^{n-1}\alpha}$ 下是多项式 $f_T$ 的友阵：
...
条件 (1) 可以看作 $V \simeq (F[x]/p)^n$. 其成立时，设 $\alpha = (f_1, \dots, f_n), \beta = (g_1, \dots, g_n)$, 取 $S$ 在第 $i$ 个分量上为：
...
$c = -2$ 时，取 $W = \operatorname{span}\set{1, (x-1)^2, (x-1)^4, \dots}$ 即可。

...
local function random_evenodd(left_bound, right_bound)
    local len = (right_bound - left_bound) >> 1 + 1
    return math.random(len) << 1 - 1
end
...
local function build_maze_fragment(matrix, xl, xr, yl, yr)
    ...
    if solid ~= 1 then
        matrix.set(random_evenodd(xl + 1, xm - 1), ym, 1)
    end
    if solid ~= 2 then
        matrix.set(random_evenodd(xm + 1, xr - 1), ym, 1)
    end
    if solid ~= 3 then
        matrix.set(xm, random_evenodd(yl + 1, ym - 1), 1)
    end
    if solid ~= 4 then
        matrix.set(xm, random_evenodd(ym + 1, yr - 1), 1)
    end
end
...
local function build_maze_chunk(matrix, x, y)
    ...
    build_maze_fragment(matrix, 1, 1, 31, 31)
    matrix.set(0, random_evenodd(1, 31), 0)
    matrix.set(random_evenodd(1, 31), 0, 0)
end
